...
for i in dev uat prod; do
  mkdir -p build/$i
  cp -pr src/* $i/
  cat src/GBIF-tile-schemas.js src/GBIF-layers.js >! $i/GBIF-both.js
done

perl -pi -e "s/tile.gbif.org/tile.gbif-dev.org/g" dev/**/*.js build/dev/**/*.html
perl -pi -e "s/tile.gbif.org/tile.gbif-uat.org/g" uat/**/*.js build/uat/**/*.html
